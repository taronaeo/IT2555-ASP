<script lang="ts">
  ...
  import {collection, getDocs, query, limit, orderBy, startAfter, endBefore, doc, getDoc, where } from 'firebase/firestore';
  ...
  onMount(() => {
    function handleOutsideClick(event: MouseEvent) {
        if (!event.composedPath().includes(document.getElementById('filterDropdown') as Node)) {
  ...
  <div class="inline-flex relative pb-4">
    <ul class="flex flex-wrap px-3 py-1 bg-emerald-600 rounded-md text-base">
      <li class="relative mx-1 px-1 py-2 bg-emerald-600 rounded-full mb-1 md:mb-0">
        <button class="px-4 py-2 bg-emerald-600 text-white font-bold rounded-md focus:outline-none" on:click|stopPropagation={toggleFilters}>Filter By</button>
        {#if $showFilters}
          <ul id="filterDropdown" class="absolute top-0 left-0 mt-10 p-2 rounded-lg shadow-lg bg-white z-10">
            <li class="p-1 whitespace-no-wrap rounded-full text-sm md:text-base text-gray-600 hover:text-gray-800 hover:bg-gray-100">
              <button class="px-2 py-1 text-left" on:click={() => filterByChronological('chronological')}>
            ...
            <li class="p-1 whitespace-no-wrap rounded-full text-sm md:text-base text-gray-600 hover:text-gray-800 hover:bg-gray-100">
              <button class="px-2 py-1 text-left" on:click={() => filterByAmount('amount')}>
        ...
        <div class="container mx-auto font-thin text-">
          ...
          <li class="flex justify-between gap-x-6 py-5">
            ...
            <div
              ...
              <p class="text-sm leading-6 text-emerald-600">Amt: ${receipt.total}</p>
            </div>
          </li>
        </div></a>
      {/each}
