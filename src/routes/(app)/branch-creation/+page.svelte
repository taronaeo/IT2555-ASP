<script lang="ts">
  ...
  function next_page() {
    ...
    if (differenceFinalEndIndex >= 6) {
      for (let i = end_index; i < start_index + 5; i++) {
        end_index += 1;
      }
    } else {
      for (let i = end_index; i <= final_index - 1; i++) {
        end_index += 1;
      }
    }
    get_branches();
  ...
  async function genApiKey(branchId: string) {
    ...
    for (let i = 0; i < length; i++) {
      const randomIndex = Math.floor(
        (randomValues1[i] / 256) * charactersLength
      );
    ...
    for (let i = 0; i < length; i++) {
      const randomIndex = Math.floor(
        (randomValues2[i] / 256) * charactersLength
      );
...
<div class="px-6 md:w-2/3 m-auto">
  <div>
    <p
      class="flex flex-row text-xl font-bold mb-6 md:mx-8
    ...
    <div class="grid grid-cols-2 text-sm gap-2">
      ...
      {:else}
        {#each displayed_branches as branch}
          <button
            role="button"
            disabled={!selectBranches}
            class:bg-slate-200={selectBranches}
            on:click={() => {
              hasKey(branch['branchId']);
              inputBranchId = '';
              validRevoke = false;
              validGeneration = false;
            }}
            class="flex flex-col text-left border-2 p-2">
            <ul>
              <li class="break-all"
                ><span class="font-bold">ID:</span> {branch['branchId']}</li>
              <li class="break-all"
                ><span class="font-bold">Location:</span>
                {branch['branchLocation']}</li>
              <li class="break-all"
                ><span class="font-bold">Postal Code:</span>
                {branch['branchPostal']}</li>
            </ul>
          </button>
        {/each}
      {/if}
    ...
    <div
      class:hidden={!manageKey}
      class="flex fixed justify-center items-center inset-0 w-full bg-black/50">
      {#if !branchHasKey}
        <div
          class:hidden={deleting}
          class="text-center text-md py-2 w-10/12 md:w-2/3 lg:w-2/3 xl:w-1/3 px-4 font-bold bg-white border-2">
          <p> No API Key or Secret Created Yet. </p>
          <p
            class=" flex flex-row font-normal md:mx-8
                  before:mr-4 before:my-auto before:flex-1 before:border-b-2 before:border-dashed before:border-gray-400
                  after:ml-4 after:my-auto after:flex-1 after:border-b-2 after:border-dashed after:border-gray-400">
          ...
          <div class="text-center">
            <div class="font-normal text-md mb-2"
              >Type in Branch ID to confirm <span class="font-bold"
                >Generation</span
              ></div>
            <input
              bind:value={inputBranchId}
              on:input={() => {
                validBranchIdGen(inputBranchId, managingBranchId);
              }}
              class="mb-4 border-2 w-3/4" />
          </div>
          <button
            disabled={!validGeneration}
            on:click={() => {
              genApiKey(managingBranchId);
            }}
          ...
          <p
            class=" flex flex-row font-normal md:mx-8 my-2
              before:mr-4 before:my-auto before:flex-1 before:border-b-2 before:border-dashed before:border-gray-400
              after:ml-4 after:my-auto after:flex-1 after:border-b-2 after:border-dashed after:border-gray-400">
            Manage Branches
          </p>
        ...
        <div
          ...
          class="text-center text-md py-2 w-10/12 md:w-2/3 lg:w-2/3 xl:w-1/3 px-4 font-bold bg-white border-2">
          <p
            class=" flex flex-row font-normal my-4 md:mx-8
            before:mr-4 before:my-auto before:flex-1 before:border-b-2 before:border-dashed before:border-gray-400
            after:ml-4 after:my-auto after:flex-1 after:border-b-2 after:border-dashed after:border-gray-400">
            Delete Branch
          </p>
          ...
          <div class="text-center">
            ...
            <input
              bind:value={inputBranchId}
              on:input={() => {
                validBranchIdDel(inputBranchId, managingBranchId);
              }}
              class="mb-4 border-2 w-3/4" />
          </div>
          <button
      ...
      {:else}
        <div
          class:hidden={deleting}
          class="p-4 w-10/12 md:w-2/3 lg:w-2/3 xl:w-1/3 font-bold bg-white border-2">
          <p
            class="flex flex-row font-normal
                  before:mr-4 before:my-auto before:flex-1 before:border-b-2 before:border-dashed before:border-gray-400
                  after:ml-4 after:my-auto after:flex-1 after:border-b-2 after:border-dashed after:border-gray-400">
            Revoke API Key for:
          </p>
          ...
          <div
            class="border-2 border-gray-600 py-1 px-2 mt-2 mb-6 mx-1 md:mx-4">
            <div class="font-normal italic"
              ><span class="font-bold not-italic"
                >API Key:
              </span>{managingBranchKey}</div>
            <div class="font-normal italic"
              ><span class="font-bold not-italic"
                >API Secret:
              </span>{managingBranchSecret}</div>
          </div>
          ...
          <div class="text-center mt-2">
            ...
            <input
              bind:value={inputBranchId}
              on:input={() => {
                validBranchIdRev(inputBranchId, managingBranchId);
              }}
              class="mb-4 mt-2 border-2 w-3/4" />
          </div>
          <div class="text-center">
            <button
              disabled={!validRevoke}
              on:click={() => {
                revokeKey(managingBranchId);
              }}
              class="px-5 py-2.5 rounded-lg
                          text-white text-center font-medium
                          bg-emerald-600 hover:bg-emerald-700
            ...
            <p
              class=" flex flex-row font-normal md:mx-8 my-4
              before:mr-4 before:my-auto before:flex-1 before:border-b-2 before:border-dashed before:border-gray-400
              after:ml-4 after:my-auto after:flex-1 after:border-b-2 after:border-dashed after:border-gray-400">
              Manage Branches
            </p>
  ...
  <div>
    <p
      class="flex flex-row text-xl font-bold mb-4
      before:mr-4 before:my-auto before:flex-1 before:border-b-2 before:border-dashed before:border-gray-400
      after:ml-4 after:my-auto after:flex-1 after:border-b-2 after:border-dashed after:border-gray-400">
      Create Branch
    </p>
    <div
      ...
      class="mb-4 p-4 col-span-1 md:col-span-2 flex flex-row gap-1 text-sm text-red-800 bg-red-100 rounded-lg"
      role="alert">
      <div class="w-5 h-5">
