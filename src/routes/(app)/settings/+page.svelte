...
async function saveSettings() {
  ...
  try {
    ...
    if (serverResponse.displayName === true && serverResponse.email === true && serverResponse.phoneNumber === true) {
      const docRef = doc(firestore, 'users', userUid);
      const newData = {
        displayName: user.displayName,
        email: user.email,
...
<main class="min-h-screen flex justify-center items-center p-4">
  <div class="w-full max-w-xl bg-white rounded-lg shadow-md">
    <h1 class="text-2xl font-semibold text-center pt-6">Settings</h1>
